...
class AssemblyInfoFetcher:
    def __init__(self, assembly_target_url: str):
        ...
        self._init_const_queries()
        self.requests_headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Safari/537.36'
        }

    def _init_const_queries(self):
        self.queries["cmd"] = "movieInfo"
        self.queries["no"] = ""
    ...
    def fetch_assembly_video_info(self) -> VideoInfo:
        query_string = self._to_query_string({"vv": f"{self._get_timestamp()}"}, ["menu"])
        ...
        if result.status_code != 200:
            raise ConnectionError(f"Error fetching assembly video info: {result.status_code}/{result.text}")
